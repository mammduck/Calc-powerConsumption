...
MeasurementTargetName = 'AFSK'    #check

#import the measured value by DMM   #check csv-name
data = pd.read_csv('2021-02-18-0006_AFSK-medium-bench10.csv', names=('time',column_name1, column_name2), skiprows=2,header=None,usecols=[0,2,5], dtype={'time': str, column_name1: float, column_name2: float})
...
for i in range(len(passedTime)-1):
    #print((passedTime[i+1]-passedTime[i])*(data.iloc[i,1]+data.iloc[i+1,1])/2*(data.iloc[i,2]+data.iloc[i+1,2])/2)
    #powerConsumption += (passedTime[i+1]-passedTime[i])*(data.iloc[i,1]+data.iloc[i+1,1])/2*(data.iloc[i,2]+data.iloc[i+1,2])/2
